...
pander::panderOptions('table.split.table', 150)
options(width = 150)
...
data_frame(parameter = rownames(impact_ne_w$b), 
           ne_rma_per_change  = round(as.vector(get_percent_change(impact_ne_w$b)), digits = 4), 
           ne_rma_pval  = round(impact_ne_w$pval, digits = 4), 
           all_rma_per_change = round(as.vector(get_percent_change(impact_all_w$b)), digits = 4), 
           all_rma_pval = round(impact_all_w$pval, digits = 4), 
           #parameter_lme = rownames(summary(impact_lme_ne)$tTable), 
           lme_ne_per_change  = round(as.vector(get_percent_change(summary(impact_lme_ne)$tTable[, 1])), digits = 4), 
(       round    lme_ne_pval  = summary(impact_lme_ne)$tTable[, 5], ), digits = 4
           lme_all_per_change = round(as.vector(get_percent_change(summary(impact_lme_all)$tTable[, 1])), digits = 4), 
           lme_all_pvalround (= summary(impact_lme_all)$tTable[, 5], digits = 4
            )          ) %>%
pandoc.table(.)
...
data_frame(parameter = rownames(global_full_all_drivers$b), 
           all_drivers_percent  = round(as.vector(get_percent_change(global_full_all_drivers$b)), digits = 4), 
           all_drivers_pval  = round(global_full_all_drivers$pval, digits = 4)
           ) %>% 
