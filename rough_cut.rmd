...
data <- read.csv("Data/full_data_with_impacts_velocity_invs_fert_pest20150921.csv")
...
data <- filter(distinct(data))

# Remove data with duration == -1 :| wth.
data <- filter(data, Duration != -1)
...
filter(data, !is.na(yi.SppR.ROM) & !is.na(vi.SppR.ROM) & 
                     vi.SppR.ROM > 0 & !is.na(mean_imps)) %>%
    lmer(yi.SppR.ROM ~ Duration * (raster_vel + raster_lin + mean_invs + mean_pest) + (1 | Study.ID), data = .)
summary(global2_lme)
# scaling?
...
### Making the prediction map for just a subset of the cumulative impact data
...
names(crop_imp) <- 'mean_imps'
imp_preds <- stack(time_map1, crop_imp)
