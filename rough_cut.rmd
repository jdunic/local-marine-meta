...
ggplot(data = event, aes(x = Duration, y = yi.SppR.ROM)) + 
  geom_point(aes(colour = Expected.Change.Direction)) + 
  ylim(c(-2, 2))
...
rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM, data = event,
       ...
       mods = ~ Duration*Expected.Change.Direction)
localmod
...
rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM, data = event,
       random = ~ 1 | factor(Study.ID),
       mods = ~ Duration:Expected.Change.Direction + 
              Expected.Change.Direction + 0)
local2

local2U <- lmer(yi.SppR.ROM ~ Duration:Expected.Change.Direction + 
                Expected.Change.Direction + 0 + (1|Study.ID), data=event)
summary(local2U)
