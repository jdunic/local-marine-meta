...
# Remove after re-running masterDataCleanSort.R
fl_combined <- 
  mutate(fl_combined, Event. = replace(Event., (Study.ID == 554 & Site == "downcurrent reference"), 'No')) %>%
  mutate(Event. = replace(Event., (Study.ID == 554 & Site == "upcurrent reference"), 'No')) %>% 
  mutate(Event. = replace(Event., (Study.ID == 607 & Site == "gravelines"), 'No'))
...
In the weighted analysis, species richness did not change on average (mod1, mod2), nor was there a relationship between change in species richness and duration (mod1).

In the intercept: this is the average propotion of change in species richness, over time (intrinsically built into the metric final / initial). In the unweighted analysis, the intercept maybe be significant; the intercept estimate it double the standard error of the intercept. These results suggest that on average, regardless of duration, species richness has increased by one percent in the studies examined. These findings are similar to those found by Vellend et al 2014.
...
lme1 <- lmer(yi.SppR.ROM ~ Duration + (1 | Study.ID), data = fl_combined, REML = F)
#lme1 <- lmer(yi.SppR.ROM ~ (1 | Study.ID), data = fl_combined)
summary(lme1)

# Leave out duration (not significant)
lme2 <- lmer(yi.SppR.ROM ~ (1 | Study.ID), data = fl_combined, REML = F)
summary(lme2)

# I'm kind of surprised that the more simple model doesn't fit better... why?
anova(lme1, lme2)

```

Looking at the caterpillar plot for each of the studies, it is probably best to keep Study.ID as a random 
effect.
...
local2U <- lmer(yi.SppR.ROM ~ Duration:expected_change + 
                expected_change + 0 + (1|Study.ID), data=event)
summary(local2U)
...
ggplot(data = event_dir_summ_df[1:3, ]) +
  geom_point(aes(x = row.names(event_dir_summ_df[1:3, ]), y = mean_estimate), size = 4, 
             colour = 'white') +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci, 
                    x = row.names(event_dir_summ_df[1:3, ]), width = 0), colour = 'white', 
                    size = 1) +
  theme_wb() + 
  ...
  theme(axis.text.y = element_text(size = 18), 
        ...
        axis.title.x = element_text(vjust = 1.6, size = 20),
        axis.title.y = element_text(vjust = -0.5, size = 20), 
        panel.grid.major  = element_blank(),
        panel.grid.minor  = element_blank()) +
  geom_hline(x = 0, linetype = "dashed", colour = 'yellow') +
  ylim(c(-0.75, 0.75)) +
...
ggplot(data = event_dir_summ_df[4:6, ]) +
  geom_point(aes(x = row.names(event_dir_summ_df[4:6, ]), y = mean_estimate), size = 4, 
             colour = 'white') +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci, 
                    x = row.names(event_dir_summ_df[4:6, ]), width = 0), colour = 'white', 
                    size = 1) +
  theme_wb() + 
  ylab('Change in log ratio / year\n') +
  xlab('\nEvent Type\n') + 
...
```{r}
local_lme <- 
lmer(yi.SppR.ROM ~ Duration + expected_change + (1 | Study.ID), data = event)
...
filter(fl_combined, !is.na(yi.SppR.ROM) & !is.na(vi.SppR.ROM) & 
                     vi.SppR.ROM > 0 & !is.na(mean_imps)) %>%
  lmer(yi.SppR.ROM ~ Duration * scale(mean_imps) + (1 | Study.ID), data = .)
summary(global_lme)
