...
fl_combined <- read.csv("Data_outputs/nov_5_fl_combined_for_wsn.csv")

#fl_combined <- read.csv("Data_outputs/full_data_with_impacts_velocity_invs_fert_pest20151102.csv")
# No need for the different taxonomic 1/0 categories:
fl_combined <- fl_combined[, setdiff(names(fl_combined), c('coral', 'plant', 'algae', 'fish', 
                      'inverts', 'mobile.inverts', 'sessile.inverts', 
                      'marine.mammals', 'phytoplankton', 'zooplankton'))]


# More clean up... grrr - there can't be a species richness error = 0. This 
# just needs to be removed for study 47. 
fl_combined$vi.SppR.ROM[which(fl_combined$vi.SppR.ROM == 0)] <- NA

# Remove duplicate data
fl_combined <- filter(distinct(fl_combined, Study.ID, Site, taxa))

# Make Study.ID a factor
fl_combined$Study.ID <- as.factor(fl_combined$Study.ID)
