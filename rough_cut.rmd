...
#### THIS CHUNK IS QUITE BROKEN: L. 95 - 127
event_dir_summ_df <- 
  mk_summary_df(event, rma_object = local, moderators = FALSE, 
                mods = 'Expected.Change.Direction')
row.names(event_dir_summ_df) <- c(' Duration*Negative', ' Duration', ' Duration*Positive', ' Duration*Uncertain')
...
ggplot(data = event_dir_summ_df) +
  ...
  xlim(c(-0.75, 0.75)) +
  scale_y_discrete(labels = paste(row.names(event_dir_summ_df), "\n studies = ", as.character(num_studies$n), "\n sites = ", as.character(num_sites$n)))
...
global_summ_df <- 
  mk_summary_df(fl_combined, rma_object = global1, moderators = FALSE, 
                mods = c('mean_imps', 'Duration'))
...
filter(data, !is.na(yi.SppR.ROM) & !is.na(vi.SppR.ROM) & 
    ...
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = ., 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (scale(raster_vel) + scale(raster_lin) + scale(mean_invs) + scale(mean_pest) + scale(mean_fert))
)
global2

global_summ_df <- 
  mk_summary_df(data, rma_object = global2, moderators = FALSE)
...
filter(data, !is.na(yi.SppR.ROM) & !is.na(vi.SppR.ROM) & 
                     vi.SppR.ROM > 0 & !is.na(mean_imps)) %>%
    lmer(yi.SppR.ROM ~ Duration * (scale(raster_vel) + scale(raster_lin) + scale(mean_invs) + scale(mean_pest) + scale(mean_fert)) + (1 | Study.ID), data = .)
summary(global2_lme)
# scaling?
