...
filter(fl_combined, !is.na(yi.SppR.ROM) & !is.na(vi.SppR.ROM) & 
    ...
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = ., 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (lin_change_specific + mean_invs +  mean_fert)
)
global2
...
global2_lme <- 
lmer(yi.SppR.ROM ~ Duration * (scale(raster_vel) + scale(raster_lin) + scale(mean_invs)  + scale(mean_fert)) + (1 | Study.ID), data = fl_combined)
summary(global2_lme)
# scaling?
