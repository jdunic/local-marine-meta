...
# Find the earliest date (which would be the first date)
# Get all the dates that have a month matching the earliest date month (+/- 1)
# Get latest date in all dates
# Is the latest date with the matching month in a T1 column? (if I get yesses, 
# then I'll add in a fix for that later)
# Get the index of the earliest date
# Get the index of the latest date
# Bind the site info and richness data for the earliest date with the richness
# data for the latest date
# cbind(adf[first, c(noYCols, y1Cols)], adf[last, y2Cols])

sampleFL <- function(adf) {
    #browser()
    all_dates <- c(adf$date1, adf$date2)
    sorted <- sort(all_dates)
    first <- sorted[1]
    first_id <- 1
    month_id <- which(month(sorted) == month(first) & sorted != first)
    if (length(month_id) == 0) {
      month_id <- which(month(sorted) == month(first %m+% months(1)) | 
                        month(sorted) == month(first %m-% months(1)) 
                       )
      if (length(month_id) == 0) {
        message <- paste('First-last sampling months do not match within +/- 1 month.', 
                         'Skipping to next data subset.', 
                         'Study:', adf$Study.ID[1], 'Site:', adf$Site, '--excluded.', 
                         sep = ' ')
        print(message)
        return(data.frame(NULL))
      }
    }
    max_end_date <- max(sorted[month_id])
...
#  richData, 
#  .(Study.ID, Reference, Sys, coral, plant, algae, fish, inverts, mobile.inverts,
...
#      print(paste0('first: ', adf$date1[first_id], ' last: ', adf$date2[last_id]))
#    }
#    cbind(adf[first_id, c(noYCols, y1Cols)], adf[last_id, y2Cols])
#  }
#)
