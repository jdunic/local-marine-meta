...
lme1 <- lme(yi.SppR.ROM ~ Duration, random = ~ 1 | factor(Study.ID), 
...
impact_lme_ne <- 
  lme(yi.SppR.ROM ~ Duration * mean_imps, random = ~ 1 | factor(Study.ID),
...
data_frame(parameter = rownames(impact_ne_w$b), 
           ne_rma_per_change  = as.vector(get_percent_change(impact_ne_w$b)), 
           ne_rma_pval  = impact_ne_w$pval, 
           all_rma_per_change = as.vector(get_percent_change(impact_all_w$b)), 
           all_rma_pval = impact_all_w$pval, 
           #parameter_lme = rownames(summary(impact_lme_ne)$tTable), 
           lme_ne_per_change  = as.vector(get_percent_change(summary(impact_lme_ne)$tTable[, 1])), 
           lme_ne_pval  = summary(impact_lme_ne)$tTable[, 5], 
           lme_all_per_change = as.vector(get_percent_change(summary(impact_lme_all)$tTable[, 1])), 
           lme_all_pval = summary(impact_lme_all)$tTable[, 5]
           ) %>%
pandoc.table(.)
...
global_ne_all_drivers <- 
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = no_event, 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (mean_invs + mean_vocc + mean_lin_change + mean_nuts)
)
global_ne_all_drivers
...
global_ne_no_vocc <- 
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = no_event, 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (mean_invs + mean_lin_change + mean_nuts)
)
global_ne_no_vocc
...
global_ne_no_ltc <- 
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = no_event, 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (mean_invs + mean_vocc + mean_nuts)
)
global_ne_no_ltc

data_frame(parameter = rownames(global_ne_all_drivers$b), 
           all_drivers_percent  = as.vector(get_percent_change(global_ne_all_drivers$b)), 
           all_drivers_pval  = global_ne_all_drivers$pval
           ) %>% 
  left_join(., data_frame(parameter = rownames(global_ne_no_vocc$b), 
                         no_vocc_percent = as.vector(get_percent_change(global_ne_no_vocc$b)), 
                         no_vocc_pval = global_ne_no_vocc$pval)) %>%
  left_join(., data_frame(parameter = rownames(global_ne_no_ltc$b), 
                         no_ltc_percent = as.vector(get_percent_change(global_ne_no_ltc$b)), 
                         no_ltc_pval = global_ne_no_ltc$pval)) %>%
pandoc.table(.)
...
data_frame(parameter = rownames(global_full_all_drivers$b), 
           all_drivers_percent  = as.vector(get_percent_change(global_full_all_drivers$b)), 
           all_drivers_pval  = global_full_all_drivers$pval
           ) %>% 
  left_join(., data_frame(parameter = rownames(global_full_no_vocc$b), 
                         no_vocc_percent = as.vector(get_percent_change(global_full_no_vocc$b)), 
                         no_vocc_pval = global_full_no_vocc$pval)) %>%
  left_join(., data_frame(parameter = rownames(global_full_no_ltc$b), 
                         no_ltc_percent = as.vector(get_percent_change(global_full_no_ltc$b)), 
                         no_ltc_pval = global_full_no_ltc$pval)) %>%
pandoc.table(.)
...
global_ne_all_interactions <- 
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = no_event, 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (mean_invs + mean_vocc + mean_invs*mean_lin_change + mean_lin_change + mean_nuts + mean_invs*mean_nuts + mean_lin_change*mean_nuts)
)
...
global_all_invs_ltc_interaction <- 
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = fl_combined, 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (mean_invs + mean_invs*mean_lin_change + mean_lin_change + mean_nuts + mean_vocc)
)
global_all_invs_ltc_interaction


global_ne_all_interactions_lme <- 
  lme(yi.SppR.ROM ~ Duration * (mean_invs + mean_vocc + mean_invs*mean_lin_change + mean_lin_change + mean_nuts + mean_invs*mean_nuts + mean_lin_change*mean_nuts), 
      random = ~ 1 | factor(Study.ID),
      data = no_event, na.action = na.omit)
...
global_all_invs_ltc_interaction_lme <- 
  lme(yi.SppR.ROM ~ Duration * (mean_invs + mean_vocc + mean_invs*mean_lin_change + mean_lin_change + mean_nuts), 
      random = ~ 1 | factor(Study.ID),
      data = fl_combined, na.action = na.omit)
...
data_frame(parameter = rownames(global_all_invs_ltc_interaction$b), 
           rma_interact_change  = as.vector(get_percent_change(global_all_invs_ltc_interaction$b)), 
           rma_interact_pval  = global_all_invs_ltc_interaction$pval
           ) %>% 
  left_join(., data_frame(parameter = rownames(summary(global_all_interactions_lme)$tTable), 
                         all_full_interact_change = as.vector(get_percent_change(summary(global_all_interactions_lme)$tTable[, 1])), 
                         all_full_interact_pval = as.vector(summary(global_all_interactions_lme)$tTable[, 5]))) %>%
  left_join(., data_frame(parameter = rownames(summary(global_all_invs_ltc_interaction_lme)$tTable), 
                         all_invs_ltc_int_change = as.vector(get_percent_change(summary(global_all_invs_ltc_interaction_lme)$tTable[, 1])), 
                         all_invs_ltc_int_pval = as.vector(summary(global_all_invs_ltc_interaction_lme)$tTable[, 5]))) %>% 
pandoc.table(.)
...
initial_richness_ne <- 
    rma.mv(yi = yi.SppR.ROM, V = vi.SppR.ROM,
          data = no_event, 
          random = ~ 1 | Study.ID, 
          mods = ~ Duration * (mean_vocc + mean_lin_change + mean_nuts) * SppR1 + mean_invs*Duration
)
initial_richness_ne
...
initial_richness_ne_lme <- 
  lme(yi.SppR.ROM ~ Duration * (mean_vocc + mean_lin_change + mean_nuts) * SppR1 + mean_invs*Duration, 
      random = ~ 1 | factor(Study.ID),
      data = no_event, na.action = na.omit)
...
```{r}
global_lme_ne <- 
  lme(yi.SppR.ROM ~ Duration * (mean_vocc + mean_invs +  mean_nuts), random = ~ 1 | factor(Study.ID),
      data = no_event, na.action = na.omit)
...
global_lme_ne <- 
  lme(yi.SppR.ROM ~ Duration * (single_lin_change + mean_invs +  mean_nuts), random = ~ 1 | factor(Study.ID),
      data = no_event, na.action = na.omit)
summary(global_lme_ne)
...
# VOCC
global_lme_ne <- 
  lme(yi.SppR.ROM ~ Duration * (single_vel + mean_invs +  mean_nuts), random = ~ 1 | factor(Study.ID),
      data = no_event, na.action = na.omit)
summary(global_lme_ne)
