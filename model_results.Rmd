...
The model global1 uses only the studies that did not have events associated with them.
...
The model global2 uses the full dataset and the effect is reversed...
...
The unweighted analyses (again with the no_event data and the full dataset) also show the opposite pattern compared to the global1 model (with no event data). 
...
When comparing the models with no event and the full dataset the patterns are the same, but the short term effect of invasive potential is not significant when looking at the no_event subset of the data.
...
global_lme2 <- 
  lme(yi.SppR.ROM ~ Duration * (mean_lin_change + mean_invs +  mean_nuts), random = ~ 1 | factor(Study.ID),
      data = fl_combined, na.action = na.omit)
summary(global_lme2)
